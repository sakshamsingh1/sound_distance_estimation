...
class DataGenerator(object):
    def __init__(
        ...
        self._only_dist = params['only_dist']
            ...
            if self._multi_accdoa is True:
                ...
                if self._only_dist:
                    if self._perm2 or self._perm3:
                        label_shape = (self._batch_size, self._label_seq_len, 2)
                    # elif self._perm_1:
                    #     label_shape = (self._batch_size, self._label_seq_len, 1)
                    else:
                        label_shape = (self._batch_size, self._label_seq_len, 1)
                else:
    ...
    def _get_filenames_list_and_feat_label_sizes(self):
        ...
        for filename in os.listdir(self._feat_dir):
            if int(filename.split("_")[0][4:]) in self._splits: # if int(filename[4]) in self._splits: # check which split the file belongs to
    ...
    def write_output_format_file_onlyDist(self, _out_file, _out_dict, dist_and_mask=False):
        return self._feat_cls.write_output_format_file_onlyDist(_out_file, _out_dict, dist_and_mask=dist_and_mask)